...
4. [기능 명세](#4-기능-명세)
   ...
   - [4.7 CEFR 난이도 분석 및 아티클 추천](#47-cefr-난이도-분석-및-아티클-추천)
   - [4.8 학습 및 복습 시스템](#48-학습-및-복습-시스템)
   - [4.9 복습 알림](#49-복습-알림)
5. [엔티티 설계](#5-엔티티-설계)
...
| 단어장 관리 | 미구현 | 신규 도메인 |
| 일일 사용량 제한 | 미구현 | 신규 기능 |
| CEFR 난이도 분석 | 미구현 | AI 기반 CEFR 등급 자동 분석 |
| 아티클 추천/트렌딩 | 미구현 | CEFR 기반 추천 및 트렌딩 |
| 학습 및 복습 시스템 | 미구현 | 퀴즈, Spaced Repetition |
| 복습 알림 | 미구현 | 이메일/카카오톡 알림 |
...
└─────────────┘     │  ┌────────────┐  ┌─────────────┐                        │
                    ...
                    │  ┌─────┴──────────────────────────────────────────┐      │
                    │  │              REST API Layer                     │      │
                    │  │  Auth │ User │ Article │ VocabBook │ Learning  │      │
                    │  │       │      │         │ Quiz │ Notification   │      │
                    │  └─────┬──────────────────────────────────────────┘      │
                    │        │                                                 │
                    │  ┌─────┴──────────────────────────────────────────┐      │
                    │  │              Service Layer                      │      │
                    │  │  AuthService │ ArticleService │ QuizService    │      │
                    │  │  LearningService │ NotificationService │ ...   │      │
                    │  └─────┬──────────────────────────────────────────┘      │
                    │        │         │              │                         │
                    │  ┌─────┴───┐  ┌──┴──────────┐  ┌──┴──────────┐          │
                    │  │   JPA   │  │  Async       │  │  Scheduler  │          │
                    │  │  Repos  │  │  Executor    │  │  (@Scheduled)│          │
                    │  └────┬────┘  └──────┬───────┘  └──────┬───────┘         │
                    └───────┼──────────────┼─────────────────┼─────────────────┘
                            │              │                 │
                    ┌───────┴───┐   ┌──────┴──────┐   ┌─────┴────┐  ┌────────────┐
                    │PostgreSQL │   │ Gemini API  │   │  Redis   │  │  JavaMail  │
                    │  (Data)   │   │  (AI/CEFR)  │   │ (Cache/  │  │  /Kakao   │
                    │           │   │             │   │  Token)  │  │  (알림)    │
                    └───────────┘   └─────────────┘   └──────────┘  └────────────┘
```
...
│ role                  │
│ password              │
│ targetCefrLevel       │  ◀ v2.0 추가
│ notificationEnabled   │  ◀ v2.0 추가
│ notificationChannel   │  ◀ v2.0 추가
│ notificationTime      │  ◀ v2.0 추가
│ createdAt             │
│ updatedAt             │
└──────┬────────────────┘
       │ 1
       ...
       │ N                                │ N                    │ 1                 │ N
┌──────┴───────────────┐       ┌─────────┴──────────┐  ┌───────┴─────────┐  ┌──────┴──────────────┐
│    Article           │       │  VocabularyBook     │  │ LearningStreak  │  │  NotificationLog    │
│──────────────────────│       │────────────────────-│  │─────────────────│  │─────────────────────│
│ id (PK)              │       │ id (PK)             │  │ id (PK)         │  │ id (PK)             │
│ user_id (FK)         │       │ user_id (FK)        │  │ user_id (FK,UK) │  │ user_id (FK)        │
│ title                │       │ name                │  │ currentStreak   │  │ channel             │
│ content              │       │ description         │  │ longestStreak   │  │ status              │
│ originalUrl          │       │ isDefault           │  │ lastActiveDate  │  │ dueWordCount        │
│ sourceDomain         │       │ wordCount           │  │ createdAt       │  │ retryCount          │
│ isCompleted          │       │ createdAt           │  │ updatedAt       │  │ failureReason       │
│ cefrLevel            │ ◀ v2  │ updatedAt           │  └─────────────────┘  │ createdAt           │
│ difficulty           │ ◀ v2  └─────────┬───────────┘                       │ updatedAt           │
│ createdAt            │                 │ 1                                  └─────────────────────┘
│ updatedAt            │                 │
└──────┬───────────────┘                 │
       │ 1                               ├──────────────────────────────┐
       │                                 │ N                            │ N
       │ N                    ┌──────────┴────────────────┐   ┌────────┴──────────────┐
┌──────┴───────────────┐     │   VocabularyBookEntry      │   │     QuizSession       │
│  ArticleVocabulary   │     │───────────────────────────-│   │───────────────────────│
│──────────────────────│     │ id (PK)                    │   │ id (PK, UUID)         │
│ id (PK)              │     │ vocabularyBook_id (FK)     │   │ user_id (FK)          │
│ article_id (FK)      │     │ word                       │   │ vocabularyBook_id(FK) │
│ word                 │     │ definition                 │   │ quizType              │
│ definition           │     │ contextSentence            │   │ totalQuestions        │
│ contextSentence      │     │ sourceArticle_id (FK)      │   │ correctCount          │
│                      │     │ mastery              ◀ v2  │   │ accuracy              │
└──────────────────────┘     │ nextReviewAt          ◀ v2 │   │ status                │
                             │ reviewCount           ◀ v2 │   │ createdAt             │
                             │ correctCount          ◀ v2 │   │ updatedAt             │
                             │ lastReviewedAt        ◀ v2 │   └────────┬──────────────┘
                             │ createdAt                   │            │ 1
                             │ updatedAt                   │            │
                             └──────────┬─────────────────┘            │ N
...
| User 1 : N QuizSession | 사용자는 여러 퀴즈 세션을 생성할 수 있습니다 |
| VocabularyBook 1 : N QuizSession | 단어장 기반으로 퀴즈 세션이 생성됩니다 |
| QuizSession 1 : N QuizQuestion | 퀴즈 세션은 여러 문제를 포함합니다 |
| VocabularyBookEntry 1 : N QuizQuestion | 단어 항목이 퀴즈 문제의 출제 대상이 됩니다 |
| User 1 : 1 LearningStreak | 사용자당 하나의 학습 연속 기록을 관리합니다 |
| User 1 : N NotificationLog | 사용자에게 발송된 알림 이력을 관리합니다 |
...
- `BR-USER-003`: `targetCefrLevel`이 미설정인 경우 `null`로 반환합니다.
...
| targetCefrLevel | String | N | A1, A2, B1, B2, C1, C2 중 하나 |
| notificationEnabled | boolean | N | true 또는 false |
| notificationChannel | String | N | EMAIL, KAKAO, BOTH, NONE 중 하나 |
| notificationTime | String | N | HH:mm 형식, 30분 단위 (예: 09:00, 09:30) |
...
- `BR-USER-013`: `targetCefrLevel`은 A1, A2, B1, B2, C1, C2 중 하나여야 합니다.
- `BR-USER-014`: `notificationChannel`이 KAKAO 또는 BOTH인 경우, 카카오 계정 연동이 필요합니다.
- `BR-USER-015`: `notificationChannel`이 EMAIL인 경우, 이메일이 검증된 상태여야 합니다.
- `BR-USER-016`: `notificationTime`은 00:00~23:59 범위, 30분 단위여야 합니다.
...
| CF001 (INVALID_CEFR_LEVEL) | 유효하지 않은 CEFR 등급 | 400 Bad Request |
| NF002 (KAKAO_NOT_LINKED) | 카카오 미연동 상태에서 KAKAO 알림 설정 | 400 Bad Request |
| NF003 (EMAIL_NOT_VERIFIED) | 이메일 미인증 상태에서 EMAIL 알림 설정 | 400 Bad Request |
...
| difficulty | String | N | BEGINNER, INTERMEDIATE, ADVANCED 중 하나 (기본값: 사용자 프로필 기반) |
...
- `BR-ART-006`: 아티클 저장 후 비동기로 AI 단어 추출 및 CEFR 분석을 시작합니다.
- `BR-ART-007`: `isCompleted` 필드는 초기값 `false`이며, 단어 추출 완료 시 `true`로 변경됩니다.
- `BR-ART-008`: `difficulty` 미지정 시 사용자의 `targetCefrLevel` 기반으로 자동 매핑합니다. (A1-A2: BEGINNER, B1-B2: INTERMEDIATE, C1-C2: ADVANCED, 미설정 시: INTERMEDIATE)
- `BR-ART-009`: 크롤링 완료 후 Gemini AI가 본문의 CEFR 등급을 분석하여 `Article.cefrLevel`에 저장합니다.
